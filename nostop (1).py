...
# Fixed web interface: manual number inputs, persistent holdings dropdown.
# MODIFIED: trailing stop only triggers if stop > avg (guarantees profit)
...
def add_to_history(entry):
    ...
    if 'value' not in entry:
        ...
        elif entry['type'] in ('TP', 'SL', 'MANUAL', 'PYRAMID', 'AVG_DOWN') and 'pnl' in entry:
...
def can_buy(sym, price):
    ...
    if not s20 or not s200 or price <= s200:
        return False
...
def can_sell(sym):
    ...
    if not lots:
        return True
    last_add = max(lot.get('add_time', buy_time.get(sym, 0)) for lot in lots)
    return (time.time() - last_add) / 3600 >= cfg['min_hold_hours']

# ========= MODIFIED TRAILING STOP – NEVER BELOW ENTRY AVERAGE =========
def update_trailing_stop(sym, current_price, entry_avg):
    ...
    if current_price > trailing_state[sym]['highest']:
        ...
        raw_trailing = current_price * (1 - cfg['trailing_stop_pct'])
        trailing_state[sym]['trailing'] = max(entry_avg, raw_trailing)
        ...
        if price >= tp_price:
            ...
            if cfg['trailing_stop_pct'] > 0:
                if not cfg['trailing_only_after_pyramid'] or (sym in trailing_state and trailing_state[sym].get('has_pyramid', False)):
                    ...
                    # ***** FIX: only sell if stop is strictly above average entry *****
                    if price <= trail_sl and trail_sl > avg:
...
def check_entries():
    ...
    if not trading or len(positions) >= cfg['max_pos']:
        return
    avail = available_cash()
    if avail < cfg['min_trade']:
        return
    trade_val = avail * cur_size
    if trade_val < cfg['min_trade']:
        return
    bought = 0

    # ---------- 1) PYRAMID & AVERAGE-DOWN with prioritization ----------
    if cfg['build']:
        pyramid_candidates = []
        build_candidates = []
        for sym, lots in list(positions.items()):
            ...
            avg = sum(l['shares'] * l['price'] for l in lots) / total_shares
            pnl_pct = (price - avg) / avg * 100
            pyramid_count = sum(1 for l in lots if l.get('is_pyramid', False))
            if pnl_pct >= cfg['pyramid_threshold'] and pyramid_count < cfg['max_pyramids']:
                pyramid_candidates.append((sym, pnl_pct, lots, price, avg, pyramid_count))
            build_count = sum(1 for l in lots if l.get('is_build', False))
            if pnl_pct <= -cfg['build_trigger_pct'] and build_count < cfg['max_builds']:
                build_candidates.append((sym, pnl_pct, lots, price, avg, build_count))
        pyramid_candidates.sort(key=lambda x: x[1], reverse=True)
        build_candidates.sort(key=lambda x: x[1])
        for sym, pnl_pct, lots, price, avg, pyramid_count in pyramid_candidates:
            ...
            avg = sum(l['shares'] * l['price'] for l in lots) / total_shares
            pnl_pct = (price - avg) / avg * 100
            if pnl_pct < cfg['pyramid_threshold']:
                continue
            size = trade_val * cfg['pyramid_size']
            size = max(size, cfg['min_trade'])
            if size > avail:
                size = avail
            if size >= cfg['min_trade']:
                shares = size / price
                lot = {'shares': shares, 'price': price, 'is_pyramid': True, 'add_time': time.time()}
                positions[sym].append(lot)
                if sym not in trailing_state:
                    trailing_state[sym] = {'highest': price, 'trailing': price, 'has_pyramid': False}
                ...
                add_to_history({'date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'symbol': sym,
                                'type': 'PYRAMID', 'entry': round(price,2), 'shares': round(shares,2),
                                'position_size': round(size,2)})
                logger.info(f"🔺 PYRAMID {sym} @ {price:.2f} (PnL {pnl_pct:.1f}%, +${size:.2f})")
                if trade_val < cfg['min_trade']:
                    break
        for sym, pnl_pct, lots, price, avg, build_count in build_candidates:
            if bought >= cfg['max_buys'] or len(positions) >= cfg['max_pos']:
                break
            price = get_price_cached(sym) or price
            if not price:
            ...
            avg = sum(l['shares'] * l['price'] for l in lots) / total_shares
            pnl_pct = (price - avg) / avg * 100
            if pnl_pct > -cfg['build_trigger_pct']:
                continue
            size = trade_val * cfg['build_factor']
            size = max(size, cfg['min_trade'])
            if size > avail:
                size = avail
            if size >= cfg['min_trade']:
                shares = size / price
                lot = {'shares': shares, 'price': price, 'is_build': True, 'add_time': time.time()}
                positions[sym].append(lot)
                cash -= size
                bought += 1
                avail -= size
                trade_val = avail * cur_size
                add_to_history({'date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'symbol': sym,
                                'type': 'AVG_DOWN', 'entry': round(price,2), 'shares': round(shares,2),
                                'position_size': round(size,2)})
                logger.info(f"📉 AVG DOWN {sym} @ {price:.2f} (Loss {pnl_pct:.1f}%, +${size:.2f})")
                if trade_val < cfg['min_trade']:
                    break

    # ---------- 2) NEW ENTRIES from scan ----------
    ...
    for dip_info in candidates:
        ...
        if can_buy(sym, price):
            ...
            if size < cfg['min_trade']:
                continue
            shares = size / price
            lot = {'shares': shares, 'price': price, 'add_time': time.time()}
            positions[sym] = [lot]
            ...
            add_to_history({'date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'symbol': sym, 'type': 'BUY',
                            ...
                            'required_dip': get_required_dip(), 'position_size': round(size,2)})
            logger.info(f"🟢 NEW BUY {sym} @ {price:.2f} (Dip {dip:.1f}%, +${size:.2f})")
            if trade_val < cfg['min_trade']:
                break
    if bought:
        save_all_state()
...
# ===============================
#  FIXED HTML DASHBOARD – manual number inputs, persistent holdings dropdown
# ===============================
...
<title>DIP BOT – Parameter Optimizer</title>
<style>
...
.param-group h3 { font-size: 12px; margin-bottom: 6px; color: #3498db; }
.param-item { display: flex; justify-content: space-between; align-items: center; margin: 4px 0; gap: 10px; }
.param-item label { flex: 1; }
.param-item input { width: 100px; background: #1e2630; border: 1px solid #2a3440; color: #e0e5f0; padding: 2px 6px; border-radius: 3px; text-align: right; }
.btn {
...
.btn-success { background: #1e3a2a; border-color: #2ecc71; color: #2ecc71; }
.btn-danger { background: #3a1e1e; border-color: #e74c3c; color: #e74c3c; }
.scroll-list { max-height: 280px; overflow-y: auto; }
.dip-row, .history-row { display: flex; justify-content: space-between; padding: 6px 10px; border-bottom: 1px solid #1e2630; font-size: 12px; }
.split { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px; }
.panel { background: #11161f; border: 1px solid #2a3440; border-radius: 6px; overflow: hidden; }
.panel-header { background: #1a212c; padding: 6px 10px; font-weight: bold; font-size: 11px; text-transform: uppercase; border-bottom: 1px solid #2a3440; }
.holdings-table { width: 100%; border-collapse: collapse; font-size: 11px; }
.holdings-table th, .holdings-table td { padding: 6px 8px; text-align: right; border-bottom: 1px solid #1e2630; }
...
.lot-type { display: inline-block; padding: 1px 4px; border-radius: 3px; margin-right: 4px; font-size: 9px; }
.lt-base { background: #1a3a5c; }
.lt-build { background: #5c4a1a; color: #f1c40f; }
.lt-pyramid { background: #1a5c3a; color: #2ecc71; }
.collapsible { background: #11161f; border: 1px solid #2a3440; border-radius: 6px; margin-bottom: 12px; }
.collapsible-header { background: #1a212c; padding: 8px 12px; cursor: pointer; font-weight: bold; font-size: 12px; display: flex; justify-content: space-between; }
.collapsible-body { display: none; padding: 8px 12px; }
.collapsible-body.open { display: block; }
.footer { margin-top: 12px; font-size: 10px; color: #5a6a7a; text-align: center; border-top: 1px solid #1e2630; padding-top: 10px; }
...
.modal-content { background: #11161f; border: 1px solid #2a3440; padding: 20px; max-width: 300px; border-radius: 8px; text-align: center; }
</style>
</head>
<body>
<div class="container">
<div class="header">
  <div class="status"><div id="dot" class="dot d-live"></div><span class="pill p-live" id="pill">LIVE</span><strong>DIPBOT OPTIMIZER</strong></div>
  <div><button class="btn btn-primary" id="btnStart">▶ START</button> <button class="btn" id="btnStop">⏸ STOP</button> <button class="btn btn-danger" id="btnShut">⛌ SHUT</button></div>
</div>
<div class="stats-row">
...
<div class="split">
  <div class="panel"><div class="panel-header">🔽 TOP DIPS <span id="scanBadge">#0</span></div><div class="scroll-list" id="dipsList"><div>Loading...</div></div></div>
  <div class="panel"><div class="panel-header">📜 TRADE LOG <span id="histBadge"></span></div><div class="scroll-list" id="histList"><div>Loading...</div></div></div>
</div>

<div class="panel" style="margin-bottom:12px"><div class="panel-header">📊 HOLDINGS <span id="holdBadge">0</span></div>
<table class="holdings-table"><thead><tr><th>SYM</th><th>ENTRY</th><th>NOW</th><th>P&L%</th><th>VAL</th><th>E/B/P</th><th>TRAIL</th><th></th></tr></thead><tbody id="holdBody"><tr><td colspan="8">No positions</td></tr></tbody></table></div>

<div class="collapsible" id="paramColl"><div class="collapsible-header" id="paramToggle">⚙️ PARAMETER OPTIMIZER <span>▼</span></div>
<div class="collapsible-body" id="paramBody"><div id="paramEditor">Loading...</div><div style="margin-top:12px; display:flex; gap:8px;"><button class="btn btn-success" id="applyBest">🏆 Apply Best Performance (4/6/10/12)</button><button class="btn" id="resetParams">↺ Reset to Defaults</button></div></div></div>

<div class="collapsible" id="wlColl"><div class="collapsible-header" id="wlToggle">📋 WATCHLIST EDITOR <span>▼</span></div>
<div class="collapsible-body" id="wlBody"><div class="watchlist-add"><input type="text" id="wlInput" placeholder="Symbol"><button class="btn" id="wlAddBtn">+ ADD</button></div><div id="wlList">Loading...</div><button class="btn btn-warning" id="wlResetBtn">⟳ RESET TO DEFAULT</button></div></div>

<div class="collapsible" id="logColl"><div class="collapsible-header" id="logToggle">🧠 STRATEGY LOGIC <span>▼</span></div>
<div class="collapsible-body" id="logBody">Loading...</div></div>
<div class="footer"><span>SID: <span id="sid">--</span></span> | <span>PID: <span id="pid">--</span></span> | <span>Scans: <span id="scans">--</span></span> | <span>Trades: <span id="trades">--</span></span> | <span>Avg W: <span id="avgW" class="pos">--</span></span> | <span>Avg L: <span id="avgL" class="neg">--</span></span> | <span>Total P&L: <span id="totPnl">--</span></span> | <span>Up: <span id="uptime">--</span>h</span> | <button class="btn btn-danger" id="btnClear" style="font-size:10px">🗑 CLEAR</button></div>
</div>
<div class="modal" id="modal"><div class="modal-content"><strong>⚠️ Clear all history?</strong><p>Resets cash to $1000, wipes trades. Backup saved.</p><div><button class="btn btn-primary" id="confirmClear">CONFIRM</button><button class="btn" id="cancelClear">CANCEL</button></div></div></div>

<script>
let expandedSymbols = new Set();
let paramEditorRendered = false;

async function api(url, body) { const opts = body ? { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) } : { method:'POST' }; return fetch(url, opts).then(r=>r.json()).catch(()=>({})); }
...
document.getElementById('confirmClear').onclick = async () => { document.getElementById('modal').classList.remove('open'); await api('/api/clear'); setTimeout(()=>location.reload(),1500); };
function initColl(tId,bId){ const t=document.getElementById(tId); const b=document.getElementById(bId); if(t&&b) t.onclick=()=>b.classList.toggle('open'); }
initColl('paramToggle','paramBody'); initColl('wlToggle','wlBody'); initColl('logToggle','logBody');

async function loadWatchlist() {
  const d = await fetch('/api/status').then(r=>r.json());
  const wl = d.watchlist || [];
  const wlCountSpan = document.getElementById('wlCountBadge');
  if(wlCountSpan) wlCountSpan.innerText = `(${wl.length})`;
  let html = '';
...
document.getElementById('wlAddBtn').onclick = async () => { const inp=document.getElementById('wlInput'); const sym=inp.value.trim().toUpperCase(); if(!sym) return; await api('/api/watchlist/add',{symbol:sym}); inp.value=''; loadWatchlist(); };
...
async function loadParams() {
  ...
  const groups = {
    ...
    'Dip Tiers': ['dip_vix_critical','dip_vix_high','dip_vix_elevated','dip_vix_moderate','dip_vix_low'],
  ...
  for(const [g, keys] of Object.entries(groups)) {
    html += `<div class="param-group"><h3>${g}</h3>`;
    for(const k of keys) {
      if(cfg[k]===undefined) continue;
      const isBool = typeof cfg[k] === 'boolean';
      const val = cfg[k];
      html += `<div class="param-item"><label>${k}</label>`;
      if(isBool) {
        html += `<input type="checkbox" ${val?'checked':''} onchange="updateParam('${k}', this.checked)">`;
      } else {
        html += `<input type="number" step="any" value="${val}" onchange="updateParam('${k}', parseFloat(this.value))">`;
      }
      html += `</div>`;
    }
    html += `</div>`;
  }
  document.getElementById('paramEditor').innerHTML = html;
  paramEditorRendered = true;
}
window.updateParam = async (k,v) => { await fetch('/api/config', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key:k,value:v})}); };
document.getElementById('resetParams').onclick = async () => { await fetch('/api/reset_config',{method:'POST'}); paramEditorRendered=false; await loadParams(); };
document.getElementById('applyBest').onclick = async () => {
  const best = { build_trigger_pct:4.0, pyramid_threshold:6.0, trailing_stop_pct:0.10, base_tp:0.12, pyramid_size:1.0, build_factor:1.0, min_hold_hours:2, reserve:0.05, static_stop_loss_enabled:true };
  for(const [k,v] of Object.entries(best)) await updateParam(k,v);
  alert('Best performance parameters applied. They will take effect immediately.');
  paramEditorRendered=false; await loadParams();
};

async function loadLogic() {
  const d = await fetch('/api/status').then(r=>r.json());
  document.getElementById('logBody').innerHTML = `<div><strong>Entry:</strong> price > 200-SMA AND dip >= ${d.dip}% below 20-SMA. VIX ${d.vix} → ${d.vix_tier}<br><strong>TP/SL:</strong> TP=${d.tp}% SL=${d.sl}%<br><strong>Build:</strong> ${d.build?'ON':'OFF'} at -${d.build_trigger_pct}% factor ${d.build_factor}<br><strong>Pyramid:</strong> at +${d.pyramid_threshold}% factor ${d.pyramid_size}<br><strong>Trail stop:</strong> ${d.trailing_stop_pct*100}% capped at entry, only after pyramid: ${d.trailing_only_after_pyramid}<br><strong>Circuit Breaker:</strong> ${d.circuit_breaker_enabled?'ENABLED':'DISABLED'} | ${d.max_consecutive_losses} losses → pause ${d.loss_streak_cooldown}h<br><strong>Win rate:</strong> ${d.wr}% → ${d.wr_effect}</div>`;
}
...
window.manualSell = async (sym) => { if(confirm(`Sell ${sym}?`)) { const res = await api('/api/sell',{symbol:sym}); alert(res.message||(res.success?'Sold':'Error')); update(); } };
...
async function update() {
  try {
    ...
    const plEl = document.getElementById('hPl'); plEl.innerText = (d.pl>=0?'+$':'-$')+Math.abs(d.pl).toFixed(2); plEl.className = 'card-value '+(d.pl>=0?'pos':'neg');
    document.getElementById('hWr').innerText = d.wr.toFixed(1)+'%';
    ...
    const streakEl = document.getElementById('streak'); streakEl.innerText = d.loss_streak; streakEl.className = 'card-value '+(d.loss_streak>=3?'neg':d.loss_streak>=1?'warn':'pos');
    ...
    if(d.trading_paused) { banner.classList.add('show'); document.getElementById('bannerTxt').innerText = d.pause_remaining_hours>0?'resumes in '+d.pause_remaining_hours.toFixed(1)+'h':'resuming soon'; } else banner.classList.remove('show');
    let dipsHtml = ''; for(const b of (d.buys||[])) dipsHtml += `<div class="dip-row"><span class="symbol">${b.sym}</span><span class="dip-pct">▼${b.dip}%</span><span>$${b.price}</span></div>`;
    document.getElementById('dipsList').innerHTML = dipsHtml || '<div>No qualified dips</div>'; document.getElementById('scanBadge').innerText = '#'+d.scan;
    let histHtml = ''; for(const h of (d.trade_history||[]).slice().reverse()) { let typeClass='history-type'; if(h.type==='BUY') typeClass+=' type-buy'; else if(h.type==='TP') typeClass+=' type-tp'; else if(h.type==='SL') typeClass+=' type-sl'; else if(h.type==='MANUAL') typeClass+=' type-manual'; else if(h.type==='PYRAMID') typeClass+=' type-pyramid'; else if(h.type==='AVG_DOWN') typeClass+=' type-avg_down'; let val='--'; if(h.pnl!=null) val=(h.pnl>=0?'+$':'-$')+Math.abs(h.pnl).toFixed(2); else if(h.position_size) val='$'+h.position_size.toFixed(2); histHtml+=`<div class="history-row"><div><span class="${typeClass}">${h.type}</span> <strong>${h.symbol||'--'}</strong><br><span style="font-size:9px">${(h.date||'').slice(5,16)}</span></div><div>${val}</div></div>`; }
    document.getElementById('histList').innerHTML = histHtml || '<div>No trades</div>'; document.getElementById('histBadge').innerText = d.total_trades+' total';
    
    // Rebuild holdings table but preserve expanded states
    let rows = '';
    for(const h of (d.holdings||[])) {
      const pnlClass = h.pnl>=0?'pos':'neg';
      let trailHtml = (h.trail_active && h.trail_price)?`<span class="pos">🔻 $${h.trail_price}</span>`:`<span class="warn">⛔ Inactive</span>`;
      ...
      if(h.lots_detail && h.lots_detail.length>1) {
        ...
        rows += `<tr id="lots-${h.sym}" class="lot-detail-row" style="display:${disp}"><td colspan="8"><div>`;
        for(const lot of h.lots_detail) {
          let cls = 'lt-add';
          if(lot.type==='BASE') cls = 'lt-base';
          else if(lot.type==='BUILD') cls = 'lt-build';
          else if(lot.type==='PYRAMID') cls = 'lt-pyramid';
          rows += `<div><span class="lot-type ${cls}">${lot.type}</span> @$${lot.price} (${lot.shares} sh)</div>`;
        }
        rows += `</div></td></tr>`;
      }
    }
  ...
  } catch(e) { console.error(e); document.getElementById('dipsList').innerHTML = '<div>Error loading data</div>'; }
}

loadWatchlist(); loadParams(); loadLogic(); setInterval(update, 2500);
</script>
</body>
</html>'''
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        except Exception as e: logger.error(f"GET Error: {e}")
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == '/api/start':
                trading = True
                if trading_paused: trading_paused = False; loss_streak = 0
                logger.info("▶ Trading STARTED")
            elif path == '/api/stop':
                trading = False; logger.info("⏸ Trading STOPPED")
            elif path == '/api/clear':
                backup = clear_all_history(); logger.info(f"🗑️ History cleared! Backup: {backup}")
            elif path == '/api/shutdown':
                logger.info("🛑 Shutting down..."); running = False; save_all_state(); threading.Thread(target=lambda: (time.sleep(1), os._exit(0))).start()
            elif path == '/api/sell':
                symbol = post_data.get('symbol', '').upper().strip()
                if not symbol: self.send_response(400); self.end_headers(); self.wfile.write(json.dumps({'success': False, 'message': 'No symbol'}).encode()); return
                success, msg = manual_sell(symbol); self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'success': success, 'message': msg}).encode()); return
            elif path == '/api/watchlist/add':
                symbol = post_data.get('symbol', '').upper().strip()
                if symbol: add_to_watchlist(symbol); self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'status': 'ok'}).encode()); return
            elif path == '/api/watchlist/remove':
                symbol = post_data.get('symbol', '').upper().strip()
                if symbol: remove_from_watchlist(symbol); self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'status': 'ok'}).encode()); return
            elif path == '/api/watchlist/reset':
                reset_watchlist(); self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'status': 'ok'}).encode()); return
            elif path == '/api/config':
                key = post_data.get('key'); value = post_data.get('value')
                if key in cfg:
                    ...
                    cfg[key] = value; save_config(); logger.info(f"Config updated: {key} = {value}")
                    self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'status': 'ok'}).encode()); return
                ...
                cfg.clear(); cfg.update(default_cfg); save_config(); logger.info("Configuration reset to defaults")
                self.send_response(200); self.end_headers(); self.wfile.write(json.dumps({'status': 'ok'}).encode()); return
            else: self.send_response(404); self.end_headers(); return
...
def run_as_daemon():
    ...
    os.dup2(devnull.fileno(), 0); os.dup2(devnull.fileno(), 1); os.dup2(devnull.fileno(), 2)
...
def main():
    ...
    def shutdown_handler(signum, frame):
        ...
        save_all_state(); save_config(); save_watchlist()
        try:
            if os.path.exists(PID_FILE): os.remove(PID_FILE); logger.info(f"Removed PID file {PID_FILE}")
        except Exception as e: logger.error(f"Failed to remove PID file: {e}")
    ...
    except KeyboardInterrupt: shutdown_handler(None, None)
